...
def temp_horaire(machine: str) -> int:
    ...
    for ligne in data:
        ...
        if date_i.date() == today_test and date_i.hour== hour_test:
            total.append(ligne["Température (°C)"])

    print(f"Température horaire pour {machine} le {today_test} à {hour_test} : {np.mean(total):.2f} °C")
    return np.mean(total)

print(temp_horaire("Carottage"))
